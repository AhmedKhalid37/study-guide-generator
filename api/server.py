...
def _artifact_path(job: Job, artifact_name: str) -> tuple[Path, str]:
    # Sibling JSON artifacts are downloadable through the existing per-file route,
    # but intentionally NOT in ARTIFACTS - keeping them out of _artifact_urls /
    # _artifact_details means no new row appears in generic artifact UI lists.
    # Each one is reached only by an exact, fixed filename, so no path traversal
    # is introduced.
    ...
    if artifact_name == "guide_quality_report_v2.json":
        # Slice 96: Full Material Coverage MEASUREMENT report derived from the
        # generated clean.md (scanned for safe COUNTS ONLY — no excerpt persisted)
        # plus the already-sanitized coverage artifacts. Sanitized counts/tokens
        # only; calls no LLM, reconstructs no table, inspects no PDF/image. Exact-
