...
Purity & safety
---------------
stdlib-only. The plan emits **only** closed tokens, ints, ``None``, fixed strings,
and a **safe generated** ``candidate_id`` (Slice 90: a fixed-shape sequential id
such as ``"visual_candidate_0001"`` derived purely from the manifest record's
positional index — it carries no filename, path, slug, or other source detail). No
filename, path, source title, caption, document/OCR/table text, image ref, asset
ref / asset id, image byte, base64 / data URI, provider payload, token, URL, argv,
socket path, model path, or raw exception string can survive into the plan: input
fields are read for *decisions only* and never echoed. The manifest's internal
``asset_id`` is used solely for in-memory dedupe and is never emitted.

Slice 90 candidate mapping
--------------------------
Each planned item now carries a safe ``candidate_id`` generated by
:func:`candidate_id_for_manifest_position` from the record's position in the
(already page-filtered) manifest ``assets`` list. This is the *only* mapping key a
downstream consumer (the Slice 90 full non-table figure insertion path) needs to
re-resolve a planned item back to its sanitized manifest record — without the plan
ever exposing a path, filename, slug, or asset ref. The id is order-stable: it is
tied to manifest position, not to plan order, so it survives the deterministic
re-sort below.
"""
...
# Slice 90: fixed-shape, safe generated candidate-id prefix. The id is purely a
# sequential ordinal of the manifest record's position; it never encodes a slug,
# filename, path, or any source detail, so it is safe to persist in the public
# plan artifact and to use as the downstream insertion mapping key.
CANDIDATE_ID_PREFIX = "visual_candidate_"


def candidate_id_for_manifest_position(position: Any) -> str | None:
    """Safe generated candidate id for a manifest record at 0-based ``position``.

    Returns ``"visual_candidate_NNNN"`` (1-based, zero-padded to at least four
    digits) for a non-negative int position, else ``None``. Pure / total: the id is
    derived solely from the positional ordinal and carries no path, filename, slug,
    or other source detail. The same formula is used by the Slice 90 insertion path
    to map a planned item back to its sanitized manifest record.
    """
    if isinstance(position, bool) or not isinstance(position, int):
        return None
    if position < 0:
        return None
    return f"{CANDIDATE_ID_PREFIX}{position + 1:04d}"
...
def _build(visual_manifest: Any, max_items: Any) -> dict[str, Any]:
    ...
    for position, record in enumerate(assets):
        ...
        planned.append(
            {
                "candidate_id": candidate_id_for_manifest_position(position),
