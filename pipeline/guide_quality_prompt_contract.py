...
_CORE_RULES: tuple[str, ...] = (
    ...
    "Distinguish student-facing questions from model-facing questions. The guide "
    "may pose questions when they are part of student-facing learning structure — a "
    "solved mock exam, practice questions, a self-test checklist, active-recall "
    "prompts, exam-alert callouts, and section headings framed as concept questions "
    "are all allowed and encouraged. What is prohibited is the model posing its own "
    "unresolved question in explanatory prose: an explanatory sentence or paragraph "
    "must state the settled answer, never ask the question the model has not resolved "
    "and never leave a posed question unanswered from the source. Do not convert "
    "unresolved deliberation into a rhetorical question. Question marks and "
    "concept-framed headings are never banned on their own; only unresolved "
    "model-facing uncertainty is.",
    ...
    "the calculation path and state the final committed value clearly — one "
    "committed final value — with no unresolved placeholders and no speculative "
    "alternative values left open. Two or more candidate values for the same "
    "quantity may appear only when the contrast is explicitly pedagogical and "
    "unambiguous (for example a deliberately-shown wrong answer beside the correct "
    "one); never leave competing values unresolved as the final answer. If a number "
