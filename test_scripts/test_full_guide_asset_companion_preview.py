"""Public-safe tests for Slice 177C full-guide asset companion preview.

All fixtures are synthetic; no private artifacts, provider calls, or raw study
content are touched. The existing 177B combined-companion, 177A figure-companion and
176X table-companion tests are validated by their own scripts in the slice run.
"""
...
from pipeline.full_guide_asset_companion_preview import (  # noqa: E402
    ...
    run_full_guide_asset_companion_preview,
)
...
def _full_guide_md(*, extra: str = "") -> str:
    return (
...
def _make_full_guide(base: Path, *, extra: str = "") -> str:
    d = base / "full_guide"
    d.mkdir(parents=True, exist_ok=True)
    p = d / "ensemble_latest.md"
    p.write_text(_full_guide_md(extra=extra), encoding="utf-8")
    return str(p)
...
def _run(base: Path, full_guide: str | None, combined_dir: str | None) -> dict:
    return run_full_guide_asset_companion_preview(
        full_guide_path=full_guide,
        combined_companion_dir=combined_dir,
        private_preview_dir=str(base / "preview_177c"),
    )
...
def test_completed_preview_path() -> None:
    print("test_completed_preview_path")
    with tempfile.TemporaryDirectory() as tmp:
        base = Path(tmp)
        summary = _run(base, _make_full_guide(base), _make_combined_companion(base))
        html_path = base / "preview_177c" / PREVIEW_GUIDE_HTML
        html = html_path.read_text(encoding="utf-8") if html_path.is_file() else ""
    _check("status completed", summary["status"] == "completed", str(summary))
    _check("slice 177C", summary["slice"] == "177C")
    _check("artifact name", summary["artifact_name"] == "full_guide_asset_companion_preview")
    ...
    _check("combined source", summary["combined_companion_source"] == "private_177b_combined_asset_companion_guide")
    ...
    _check("preview written", summary["private_full_preview_written"] is True)
    _check("preview gitignored", summary["private_full_preview_gitignored"] is True)
    ...
    _check("next step operator read", summary["recommended_next_step"] == "operator_read_private_full_guide_asset_companion_preview")
...
def test_closed_summary_no_leak_contract() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        summary_path = base / "preview_177c" / PREVIEW_SUMMARY_JSON
...
def main() -> int:
    ...
    if _FAILURES:
        ...
        return 1
    print("\nAll full-guide asset companion preview tests passed.")
    return 0
