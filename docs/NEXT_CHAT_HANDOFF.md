...
- **177C lineage / scope.** 176X proved role-aware table simplification; 177A proved one writer-generated non-table
  ...
  document (one full-guide preview only, not all tables/figures). `module=pipeline/full_guide_asset_companion_preview.py`
  (`run_full_guide_asset_companion_preview(...)`), `runner=test_scripts/run_full_guide_asset_companion_preview.py`,
  `test=test_scripts/test_full_guide_asset_companion_preview.py`, `doc=docs/FULL_GUIDE_ASSET_COMPANION_PREVIEW.md`.
  No provider call, no generation/OCR rerun, no frontend/API, no judge, no repair, no cloud OCR, no Chandra, no broad
  OCR, no numeric verification. Preferred (and taken) path reuses the existing private full guide + accepted 177B
  combined companion verbatim.
- **177C real-run result (no provider call).** `artifact_name=full_guide_asset_companion_preview` · `slice=177C` ·
  `status=completed` · `source_label=ensemble` · `input_full_guide_available=true` · `input_full_guide_gitignored=true` ·
  `combined_companion_source=private_177b_combined_asset_companion_guide` · `combined_companion_available=true` ·
  `combined_companion_gitignored=true` · `private_full_preview_written=true` · `private_full_preview_gitignored=true` ·
  `render_format=html` · `render_status=rendered` · `normal_guide_content_present=true` ·
  `asset_companion_section_inserted=true` · `table_companion_inserted=true` · `faithful_table_present=true` ·
  `role_aware_simplified_table_present=true` · `simplified_table_is_row_reduced_copy=false` ·
  `simplified_table_is_study_oriented=true` · `figure_companion_inserted=true` · `figure_visual_present=true` ·
  `figure_visual_type_closed=flow_or_structure_diagram` · `figure_visual_is_non_table_figure=true` ·
  table/matrix/grid/text-only/sliver all false · `figure_explanation_present=true` · `figure_reading_steps_present=true` ·
  `figure_exam_takeaway_present=true` · `relative_asset_refs_preserved=true` · `raw_private_paths_in_rendered_html=false` ·
  `broken_image_marker_detected=false` · `data_image_used=false` · `base64_image_used=false` · `provider_call_made=false` ·
  `generation_rerun=false` · `frontend_api_changed=false` · `judge_ready=false` · `repair_ready=false` ·
  `blocked_by=none` · `recommended_next_step=operator_read_private_full_guide_asset_companion_preview`.
  **Operator must open the private preview HTML and confirm it reads as a full guide (not only the companion section),
  both companion blocks are visible/readable, the relative image renders, and there are no raw private paths or
  broken-image indicators.**

### Previous position (Slice 177B — committed/merged/pushed `49ee0a1`)
- **Slice 177B** composed the accepted 176X table companion and accepted 177A non-table figure companion into one
  private rendered combined companion guide. Committed and fast-forward-merged to trunk `chrome-renderer-v1`.
  `module=pipeline/combined_asset_companion_guide.py`, `doc=docs/COMBINED_ASSET_COMPANION_GUIDE.md`.
...
- **Slice 176V result (private runner over existing private artifacts).** `artifact_name=reconstructed_table_companion` ·
  `status=blocked` · `selected_asset_category=patient_dataset_table` · `selected_asset_kind=table` ·
  `faithful_table_present=true` · `simplified_table_source=deterministic_private_transform` ·
  `simplified_table_non_placeholder=true` · `explanation_source=unavailable` ·
  `explanation_non_placeholder=false` · `blocked_by=explanation_missing` ·
...
- **176V interpretation.** The mechanical faithful→explanation→simplified companion path is real and proven by
  the public-safe tests with synthetic operator content (`status=completed`, two rendered tables + explanation
  section, all canaries absent). On the real private artifacts the runner blocks honestly because the explanation
  is operator-provided private content (never synthesized) and none is staged. Provide
  `PRIVATE_TABLE_EXPLANATION_MD` (and optionally `PRIVATE_SIMPLIFIED_TABLE_MD`) to render a complete private
  companion, then inspect it privately. No automated inspection module was built; the operator's manual review is
  the inspection producer. **NOT committed.**
- **Slice 176U** added a private rendered reconstructed-table proof; operator confirmed all tables readable.
...
- **Validation for 176U.** `compileall api pipeline test_scripts` OK;
  ...
  ignored/uncommitted. **Committed `809ed1f` / merged to trunk.**
...
- **Produce-before-scaffold gate:** the required artifact is a real closed extraction run on the NN3 and
  ...
  producer exists (`pipeline.extract.extract_file`), so the project ran it instead of building another audit
  scaffold or more verified-numeric context plumbing.
- **Closed extraction provenance/semantics result:** source_decks_available=true; extraction_ran=true;
  source_identity_matches_expected_deck=true; extraction_was_run_against_raw_source=true;
  raw_source_pdf_checked_for_both_decks=true; extraction_output_is_prior_processed_text=false;
  recoverability_semantics=inputs_not_answer_strings; existing_producer_used=`pipeline.extract.extract_file`;
  OCR_path_used=existing_local_tesseract;
  table_structure_path_used=none; targets_considered=12; extraction_gated_count=0; method_gated_count=2;
  absent_or_not_found_count=0; already_recoverable_count=10; partial_recovery_count=2. NN3: partial_text,
  5/5 already recoverable by computation inputs. Ensemble: image_heavy, 5/7 already recoverable by
  computation inputs, 2/7 method-gated. Do not call the method-gated targets OCR-gated.
- **Gate 2 correction banked:** the first framing was wrong (`clean_text`/`OCR_path_used=none`). Corrected framing:
  `ensemble=image_heavy`, `nn3=partial_text`, `OCR_path_used=existing_local_tesseract`, and extraction input was
  raw source PDF rather than generated guide or prior processed text. Recovered means computation inputs sufficient
  to recompute, not answer-string presence.
- **High-risk masked recompute spot-check passed:** source_label=ensemble; target_id=`gini_weight_gt_176`;
  raw_source_checked=true; extraction_input_kind=raw_source_pdf; OCR_method_used=existing_local_tesseract;
  ...
  recoverable_by_inputs_not_answer=true; spot_check_status=passed. The answer string was masked/excluded before
  recompute; the fixture expected value was withheld from the recompute call and used only for final tolerance
  comparison.
- **Reproducibility bug:** current local closed artifact sets do not provide a complete reproducible Builder
  profile. `generation_settings_persisted_to_job_artifact=false`; `reproducible_builder_profile_available=false`;
  missing categories are `length`, `source_labels_input_mode`, and `exported_job_manifest`; impact is
  `manual_regeneration_not_reproducible`,`provenance_fragile`,`operator_equivalence_blocked`. If prioritized
  separately, the slice name should be `persist_generation_settings_to_job_artifact`.
- **Invariant going forward:** no Phase-0 product-fix slice may be committed when
  `student_visible_output_invariant=false`, unless that slice directly produces the real required artifact in
...
- **Superseded position (Slice 173A) — now committed `1905af7`.** Recompute-first PROOF slice, not generation
  ...
  **Slice 173A committed `1905af7` and merged to trunk (superseded by 173B above).**
...
- **Prior trunk lineage:** Slice 166 `e21e741`; Slice 165 `caf8171`; Slice 164 `bce7e33`; Slice 163 `c4d9607`.
  ...
  - **Slice 164 (code) integrates the existing fact-sheet schema + recompute verifier into the Phase 0 eval path** in
    ...
    `build_phase0_fact_sheet_summary(fact_sheet, golden_spec)`, pure/caller-supplied in-memory only, returns closed
    counts/statuses/warnings only (`fact_sheet_status`, `recompute_verifier_status`, verified/failed/unverified/canonical
    numeric counts). `score_phase0_layer1(..., fact_sheet=None)` now optionally folds that summary into Layer-1 scoring:
    no supplied fact sheet preserves previous numeric behavior and reports `not_supplied`/`not_run`; recompute-failed
    numeric facts force `numeric_correctness` to fail/block; verified/canonical numeric facts add committed targets the
    candidate must still contain; unverified/low-confidence numeric facts are counted/warned and never used to pass
    numeric correctness. `build_phase0_regression_record(...)` carries the closed `fact_sheet_summary` safely.
    **Input mode is caller-supplied in-memory only.** No producer, discovery, production job/API/frontend wiring, OCR,
    clean.md/raw-artifact reader, repair, judge execution, provider/model/cloud/local-LLM call, or manual operator
    `known_numbers` infrastructure was added. The factsheet-spec invariant is preserved: unverified numeric facts never
    become confident values. Tests `test_quality_safety_eval_harness.py` **254/0** so far; full validation is pending.
...
- **Slice 154 (Reasoning Leak Fix Iteration 2)** added one concise rule to the shared `_CORE_RULES` chokepoint in
  `pipeline/guide_quality_prompt_contract.py`: require direct instructional prose and forbid opening a sentence/paragraph with a
  conversational correction / reasoning discourse marker (`Actually,` / `Presumably,`), stating the corrected fact plainly
  instead. It does not ask the model to reveal hidden reasoning. Verified on a fresh app-pipeline regeneration
  (`app_run_6_reasoning_fix_iteration_2`): `reasoning_leak_status` `fail`→`pass`, contract-lint `reasoning_leak_count` `1`→`0`
  with the detector unchanged, `structure_contract_status` `warning`→`pass`. Detector, baseline aggregator, API, frontend,
  providers, renderer, exports, OCR, tables, visuals, Ask Guide, repair, and judge unchanged.

### Previously (Slice 153, now trunk `af606d4`)
- **Slice 153 (App-Pipeline Baseline Provenance Gate)** verified the full app pipeline after Slice 151 prompt hardening + Slice
  152 detector hardening: the running app was restarted on this working tree, confirmed live (`/api/health` ok), and one fresh
  NN/Iris guide generated. `runtime_refreshed_after_slice152=true`; the app's stored contract-lint artifact and the hardened
  working-tree detector both yield `reasoning_leak_count=1` (pre-fix was `2`) — app-pipeline provenance genuine. The single hit is
  a genuine line-initial discourse/intensifier marker (a true positive, **not** a detector false positive), so
  `app_run_5_app_pipeline_verified` recorded `reasoning_leak_status=fail` / `baseline_status=failed` honestly and routed the next
  fix to the prompt contract (`reasoning_leak_fix_iteration_2`). Docs-only; no code change.
...
- **Slice 150 (Reasoning Leak Baseline Failure Triage — closed-record only)** triaged the Slice 149 measured failure into a
  closed record (no code change): `app_run_1` → `contract_lint_false_positive` / `detector_boundary_false_positive`; `app_run_2`
  → `true_reasoning_leak_visible` / `internal_reasoning_phrase`; `overall_decision=true_leak_confirmed`;
  `next_step=first_measured_reasoning_leak_fix`. Baseline mapping confirmed correct (`count > 0 → fail`); generation prompts left
  unchanged pending an operator decision (now actioned in Slice 151).
...
- **Slice 148 (Judge Path Freeze / Calibration Decision Checkpoint, docs-only)** froze the synthetic offline-judge path
  (`judge_ready=false`; `repair_ready=false`; `artifact_write_ready=false`; `ui_display_ready=false`;
  `calibration_status=synthetic_only`; `private_operator_judge_calibration_run=not_run`) and redirected the next phase to a
  measured guide-quality baseline that **reuses existing wired artifacts** (corrected away from synthetic-only). It also recorded
  that privacy ≠ synthetic-only and that source/reference PDFs + generated guides stay local/gitignored while only the golden
  spec + closed aggregate metrics are committed.

### Previously (Slice 147, now trunk `effc9d9`)
- **Slice 147 (Advisory Offline Judge Artifact Schema Adapter, Synthetic Only)** added the pure, unwired adapter
  (`pipeline/quality_safety_offline_judge_artifact_adapter.py`) that adapts a normalized offline judge report (Slice 142 schema /
  Slice 143 core shape) into a write-ready synthetic artifact payload **shape** only — no artifact file written, no job wiring, no
  UI display, no judge execution, no provider/model/cloud/local-LLM call — plus a 582-check test and a synthetic harness. Payload
  rebuilt strictly through the Slice 142 normalizer so forbidden content cannot survive; readiness conservative
  (`advisory_judge_artifact_adapter_status=ok`; `artifact_shape_ready=true`; `artifact_write_ready=false`;
  `ui_display_ready=false`; `judge_ready=false`; `repair_ready=false`; `calibration_status=synthetic_only`;
...
- **Slice 145 (Private Operator Judge Calibration Pass)** added the closed-record calibration gate harness
  (`test_scripts/validate_quality_safety_judge_calibration_gate.py`) exercising the Slice 144 protocol's five golden cases against
  the pure Slice 143 offline judge core with synthetic observations only. Private/operator run was **not** performed
  (`private_operator_judge_calibration_run=not_run`); local closed-record mode is implemented but not run as committed data.
...
- **Slice 143 (Offline Judge Core v1, Synthetic Only)** implemented a pure, unwired **offline judge core**
  (`pipeline/quality_safety_offline_judge_core.py`) converting caller-supplied **synthetic closed observations** into the
  Slice 142 report shape using deterministic rules only (not an LLM judge), plus synthetic cases, a synthetic harness, and a
  769-check test. `offline_judge_core_status=ok`; `calibration_status=synthetic_only`; `judge_contract_ready=true`;
  `judge_ready=false`; `repair_ready=false`; `next_step=judge_calibration_gate_golden_protocol`.

### Previously (Slice 142, now trunk `800b187`)
- **Slice 142 (Offline Judge Schema Fixtures and Synthetic Harness)** implemented the offline judge contract as a **pure schema
  module** (`pipeline/quality_safety_offline_judge_schema.py`) plus synthetic fixtures and a synthetic harness — proving the
  future report shape can be normalized / sanitized / serialized / validated using synthetic data only (not judge execution).
  `offline_judge_schema_status=ok`; `judge_contract_ready=true`; `judge_ready=false`; `repair_ready=false`;
...
- **Slice 140 (Quality Safety Surface Cleanup / Freeze)** froze the deterministic (non-judge) Quality Safety surface (docs /
  test-surface only; no production code change) and added the consolidated freeze summary
  `docs/QUALITY_SAFETY_SURFACE_FREEZE.md`. Recorded frozen artifact names, frozen field families, the nine frozen validation
  harnesses, frozen non-goals, and known limitations: `deterministic_safety_floor_status=ready_for_cleanup_freeze`;
  `numeric_infrastructure_frozen=true`; `quality_safety_surface_frozen=true`; `judge_contract_ready=true`; `judge_ready=false`;
  `repair_ready=false`; `next_step=offline_judge_contract_design`. No production code changed.
...
- **Slice 138 (Operator Structured Numeric Export Validation Harness / Waiver Decision)** added the test/docs-only harness
  `test_scripts/validate_quality_safety_operator_structured_numeric_export.py`, which runs the Slice 137 validator end-to-end
  through the full chain and records a closed-vocabulary result. Synthetic-only run (`35 passed`); the optional local/private
  `--input` mode was implemented but not run/committed. `operator_numeric_export_waiver=approved_for_safety_floor_finalization_synthetic_only`;
  `numeric_infrastructure_frozen=true`; `judge_ready=false`; `repair_ready=false`;
  `next_step=deterministic_safety_floor_final_gate`. No production code changed.

### Previously (Slice 137, now trunk `763fb4a`)
- **Slice 137 (Pure Operator Structured Numeric Export Validator v1)** added the pure/unwired
  `pipeline/quality_safety_operator_structured_numeric_export_validator.py`, which gates an operator-authored (or synthetic)
  `quality_safety_structured_numeric_candidates`-like dict against the Slice 136 protocol and re-emits a sanitized,
  forbidden-field-free payload for the existing adapter → safe extractor → mapper → fact-sheet → recompute chain. Synthetic-only;
  no production wiring; `quality_safety_job_artifact.py` unchanged. `validator_status=ready`; `judge_ready=false`;
  `repair_ready=false`; `next_step=operator_structured_numeric_export_validation_harness`.
...
- **Slice 134 (Wire Structured Numeric Candidate Adapter into Advisory Artifact Path)** added advisory/non-blocking wiring: it
  optionally reads the exact job-local sidecar `quality_safety_structured_numeric_candidates.json` (read-only; never created),
  adapts it through the Slice 133 pure adapter, and feeds the adapted payload into the existing safe numeric extractor path.
  Precedence is deterministic, no merging: explicit records > safe candidates > structured candidates. The unified artifact adds
  `structured_numeric_candidate_adapter_{status,summary,warnings}` (count/status/closed-warning only). Synthetic clean candidates
  pass recompute; wrong supported-method candidates block; unsupported methods are counted not blocked.
...
### Previously (Slice 133, now trunk `990179c`)
- **Slice 133 (Pure Structured Numeric Candidate Artifact Adapter v1)** added the pure/unwired adapter
  `pipeline/quality_safety_structured_numeric_candidate_adapter.py` plus synthetic tests. It reads only caller-supplied dict/list
  values and emits a `quality_safety_safe_numeric_candidates` payload under the existing `candidates` wrapper. No production
  wiring, sidecar read/write, job-folder scan, source/OCR/table/caption parsing, `clean.md` numeric read, provider/model/cloud,
  judge, repair, or blocking. `adapter_status=ready`; `producer_implemented=false`; `numeric_fact_sheet_extraction_leg_status=partial`;
  `judge_ready=false`; `repair_ready=false`.
...
- **Slice 131 (Production Safe Candidate Source Discovery)** found no existing already-produced structured artifact that safely
  contains finite numeric values plus structured `computation.method` and numeric `computation.inputs`. Structural coverage,
  visual-selection, table policy/count artifacts, and `quality_safety_unified_qa` are not candidate sources. Existing
  numeric-capable paths remain sidecar-only:
  `quality_safety_numeric_extraction_records.json` and `quality_safety_safe_numeric_candidates.json`.
  `existing_production_safe_source_present=false`; `sidecar_only_source_present=true`; `judge_ready=false`;
  `repair_ready=false`; `next_step=future_structured_numeric_artifact_design`.

### Previously (Slice 130, now trunk `cd3a23b`)
- **Slice 130 (Wire Safe Numeric Extractor into Advisory Artifact Path)** wired the Slice 129 safe numeric extractor into the
  advisory `quality_safety_unified_qa.json` artifact path through an optional read-only internal candidate sidecar
  `quality_safety_safe_numeric_candidates.json`. Explicit `quality_safety_numeric_extraction_records.json` records win over safe
  candidates; safe records flow internally into `numeric_extraction_bundle`; only `safe_numeric_extractor_status`,
  `safe_numeric_extractor_summary`, and `safe_numeric_extractor_warnings` are surfaced top-level. Clean synthetic candidates pass,
  wrong synthetic candidates recompute-block, unsupported methods are counted/not blocked, and structural coverage is never
  converted into candidates. `production_numeric_extractor_present=sidecar_candidate_only`; `judge_ready=false`;
  `repair_ready=false`.
